...
public class FacilityController extends BaseController {
	...
	public void add(@Validated Facility facility, BindingResult br,
		...
		try {
			facility.setFacilityPhoto(this.handlePhotoUpload(request, "facilityPhotoFile"));
	...
	/*ajax方式按照查询条件分页查询基础设施信息*/
	@RequestMapping(value = { "/list" }, method = {RequestMethod.GET,RequestMethod.POST})
	public void list(String facilityName,String bornDate,Integer page,Integer rows, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
		if (page==null || page == 0) page = 1;
		if (facilityName == null) facilityName = "";
		if (bornDate == null) bornDate = "";
		if(rows != 0)facilityService.setRows(rows);
		List<Facility> facilityList = facilityService.queryFacility(facilityName, bornDate, page);
	    /*计算总的页数和总的记录数*/
	    facilityService.queryTotalPageAndRecordNumber(facilityName, bornDate);
	    /*获取到总的页码数目*/
	...
	public void listAll(HttpServletResponse response) throws Exception {
		...
		for(Facility facility:facilityList) {
			...
			jsonFacility.accumulate("facilityName", facility.getFacilityName());
			jsonArray.put(jsonFacility);
		}
	...
	public String frontlist(String facilityName,String bornDate,Integer currentPage, Model model, HttpServletRequest request) throws Exception  {
		if (currentPage==null || currentPage == 0) currentPage = 1;
		if (facilityName == null) facilityName = "";
		if (bornDate == null) bornDate = "";
		List<Facility> facilityList = facilityService.queryFacility(facilityName, bornDate, currentPage);
	    /*计算总的页数和总的记录数*/
	    facilityService.queryTotalPageAndRecordNumber(facilityName, bornDate);
	    /*获取到总的页码数目*/
	    int totalPage = facilityService.getTotalPage();
	...
	public void update(@Validated Facility facility, BindingResult br,
		...
		String facilityPhotoFileName = this.handlePhotoUpload(request, "facilityPhotoFile");
		if(!facilityPhotoFileName.equals("upload/NoImage.jpg"))facility.setFacilityPhoto(facilityPhotoFileName); 
	...
	public String delete(@PathVariable Integer facilityId,HttpServletRequest request) throws UnsupportedEncodingException {
		  try {
			  facilityService.deleteFacility(facilityId);
	            request.setAttribute("message", "基础设施删除成功!");
	            return "message";
	        } catch (Exception e) { 
	            e.printStackTrace();
	            request.setAttribute("error", "基础设施删除失败!");
				return "error";
	...
	/*按照查询条件导出基础设施信息到Excel*/
	@RequestMapping(value = { "/OutToExcel" }, method = {RequestMethod.GET,RequestMethod.POST})
	public void OutToExcel(String facilityName,String bornDate, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
        if(facilityName == null) facilityName = "";
        if(bornDate == null) bornDate = "";
        List<Facility> facilityList = facilityService.queryFacility(facilityName,bornDate);
        ExportExcelUtil ex = new ExportExcelUtil();
        String _title = "Facility信息记录"; 
        String[] headers = { "设施id","设施名称","设施照片","设施用途","投放日期","负责人","联系电话"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<facilityList.size();i++) {
        	Facility facility = facilityList.get(i); 
        	dataset.add(new String[]{facility.getFacilityId() + "",facility.getFacilityName(),facility.getFacilityPhoto(),facility.getPurpose(),facility.getBornDate(),facility.getFuzeren(),facility.getTelephone()});
        }
        /*
