...
public class UserInfoController extends BaseController {
	...
	public void list(String user_name,@ModelAttribute("buildingObj") Building buildingObj,String roomNo,String name,String birthDate,String telephone,String regTime,Integer page,Integer rows, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
		...
		List<UserInfo> userInfoList = userInfoService.queryUserInfo(user_name, buildingObj, roomNo, name, birthDate, telephone, regTime, page);
	    /*计算总的页数和总的记录数*/
	    userInfoService.queryTotalPageAndRecordNumber(user_name, buildingObj, roomNo, name, birthDate, telephone, regTime);
	    /*获取到总的页码数目*/
	...
	public void listAll(HttpServletResponse response) throws Exception {
		...
		for(UserInfo userInfo:userInfoList) {
			...
			jsonUserInfo.accumulate("roomNo", userInfo.getRoomNo());
			jsonArray.put(jsonUserInfo);
		}
	...
	public String frontlist(String user_name,@ModelAttribute("buildingObj") Building buildingObj,String roomNo,String name,String birthDate,String telephone,String regTime,Integer currentPage, Model model, HttpServletRequest request) throws Exception  {
		...
		if (telephone == null) telephone = "";
		if (regTime == null) regTime = "";
		List<UserInfo> userInfoList = userInfoService.queryUserInfo(user_name, buildingObj, roomNo, name, birthDate, telephone, regTime, currentPage);
	    /*计算总的页数和总的记录数*/
	    userInfoService.queryTotalPageAndRecordNumber(user_name, buildingObj, roomNo, name, birthDate, telephone, regTime);
	    /*获取到总的页码数目*/
	    int totalPage = userInfoService.getTotalPage();
	...
	public void OutToExcel(String user_name,@ModelAttribute("buildingObj") Building buildingObj,String roomNo,String name,String birthDate,String telephone,String regTime, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
        if(user_name == null) user_name = "";
        if(roomNo == null) roomNo = "";
        if(name == null) name = "";
        if(birthDate == null) birthDate = "";
        if(telephone == null) telephone = "";
        if(regTime == null) regTime = "";
        List<UserInfo> userInfoList = userInfoService.queryUserInfo(user_name,buildingObj,roomNo,name,birthDate,telephone,regTime);
        ExportExcelUtil ex = new ExportExcelUtil();
        String _title = "UserInfo信息记录"; 
        String[] headers = { "用户名","所在楼栋","房间号","姓名","性别","出生日期","业主照片","联系电话","注册时间"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<userInfoList.size();i++) {
        	UserInfo userInfo = userInfoList.get(i); 
        	dataset.add(new String[]{userInfo.getUser_name(),userInfo.getBuildingObj().getBuildingName(),userInfo.getRoomNo(),userInfo.getName(),userInfo.getGender(),userInfo.getBirthDate(),userInfo.getUserPhoto(),userInfo.getTelephone(),userInfo.getRegTime()});
        }
        /*
